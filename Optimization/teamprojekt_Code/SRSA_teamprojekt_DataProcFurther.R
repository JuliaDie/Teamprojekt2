...
if(iterative12 == 1) {
  paramsWorkers12 <- as.matrix(read.csv("teamprojekt_Data/teamprojekt_Params_simpleRSA_indOpt_iterative.csv"))
  llWorkers12 <- as.matrix(read.csv("teamprojekt_Data/teamprojekt_KLDivs_simpleRSA_indOpt_iterative.csv"))
}else {
    ...
    if(iterative12 == 1) {
      if(parSetting == 1) {
        ...
        postListMat2Opt[i,] <- determineSpeakerPostListPrefsSimpleRSAWithPriorPref(objectConstellation, featChoice,
                                                                                   abs(params12[1]),0.1, priorPrefAll_2)
...
if(iterative12 == 1) {
  if(parSetting == 1) {
    write.csv(x9data, "teamprojekt_Data/teamprojekt_dataAugm_SRSAindOpt_fixed00_andOpt12_iterative.csv")
  }else if(parSetting == 2) {
