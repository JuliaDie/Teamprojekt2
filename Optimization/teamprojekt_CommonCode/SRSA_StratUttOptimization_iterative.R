...
setwd("~/Desktop/Studienmaterial/SS20/Teamprojekt/Teamprojekt2-master (1)/Teamprojekt2-master/Utt-Pref-Choice-And-Quiz-master/Submiterator-master")
### 
  ...
  for(i in c(1:nrow(data))) {
    ...
    for(j in c(1:3)) {
      llRes <- llRes + data[i, 5+j] * 
        ( log(data[i, 5+j] + 1e-100) - log( prefPostAll[j + (data[i, 5]-1)*3] + 1e-100) ) #(observed-predicted data)
    }
