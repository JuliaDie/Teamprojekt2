...
source("teamproject_CommonCode/SRSA_StratUtt.R")
source("teamproject_CommonCode/SRSA_StratUttOptimization_iterative.R") 


# Data file from team project
x9data = read.csv(
...
##
workerIndex <- 1
for(workerID in c(0:idMax)) { #individual adjustment of parameters and KL divergence
    ...
    for(i in c(1:length(allIndices))) {
      for(j in c(1:3)) {
        llWorkers12[workerIndex, 2] <- llWorkers12[workerIndex, 2] + 
          ...
          (log(subjectResponses[allIndices[i],j]) - log(1/3)) # simple predictions
      } 
    }
    ## done with this worker -> proceed
    workerIndex <- workerIndex + 1
  }
} #shows that subjects are intelligent, as the KL divergence >> 0
...
for(workerID in c(0:idMax)) {
  ...
  if(length(idICases)>0) {
    ...
    ######################### recording results ################################# 
    ## 1 param RSA model
    llWorkers12[workerIndex,3] <- optRes1$objective #KL divergence value
    llWorkers12[workerIndex,4] <- optRes2$objective
    ## resulting parameter choice
    paramsWorkers12[workerIndex,2] <- optRes1$minimum #softness parameter value
...
# if(procType == 1) {
write.csv(llWorkers12, "teamproject_Data/x9KLDivs_simpleRSA_indOpt_iterative.csv")
write.csv(paramsWorkers12, "teamproject_Data/x9Params_simpleRSA_indOpt_iterative.csv")
# } else {
